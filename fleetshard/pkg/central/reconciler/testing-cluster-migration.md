...
url=$(infractl artifacts jmalsam-migration-cluster-1 --json | jq '.Artifacts[] | select(.Name=="kubeconfig") | .URL' -r)
wget -O ~/kubes/cluster1 $url

url=$(infractl artifacts jmalsam-migration-cluster-2 --json | jq '.Artifacts[] | select(.Name=="kubeconfig") | .URL' -r)
