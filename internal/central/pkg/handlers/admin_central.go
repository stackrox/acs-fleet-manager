...
type AdminCentralHandler interface {
	...
	// PatchCloudAccountParameters changes the cloud account of a central
	PatchCloudAccountParameters(w http.ResponseWriter, r *http.Request)
}
...
func (h adminCentralHandler) Create(w http.ResponseWriter, r *http.Request) {
	...
	cfg := &handlers.HandlerConfig{
		...
		Action: func() (interface{}, *errors.ServiceError) {
			...
			// TODO(mclasmeier): Do we need PresentCentralRequestAdminEndpoint?
...
func (h adminCentralHandler) Get(w http.ResponseWriter, r *http.Request) {
	cfg := &handlers.HandlerConfig{
		Action: func() (i interface{}, serviceError *errors.ServiceError) {
			...
			return presenters.PresentCentralRequestAdminEndpoint(centralRequest, h.accountService)
		},
	}
	...
	cfg := &handlers.HandlerConfig{
		Action: func() (interface{}, *errors.ServiceError) {
			...
			for _, centralRequest := range centralRequests {
				converted, err := presenters.PresentCentralRequestAdminEndpoint(centralRequest, h.accountService)
				if err != nil {
					return nil, err
				}
...
func (h adminCentralHandler) PatchName(w http.ResponseWriter, r *http.Request) {
	...
	cfg := &handlers.HandlerConfig{
		...
		Validate: []handlers.Validate{
			...
			ValidCentralClusterName(&updateNameRequest.Name, "name"),
			ValidateCentralClusterNameIsUnique(r.Context(), &updateNameRequest.Name, h.service),
...
func (h adminCentralHandler) PatchCloudAccountParameters(w http.ResponseWriter, r *http.Request) {
	var request *private.CentralCloudAccountChangeRequest
	cfg := &handlers.HandlerConfig{
		MarshalInto: &request,
		Action: func() (i any, serviceError *errors.ServiceError) {
			return nil, h.service.ChangeCloudAccount(r.Context(), mux.Vars(r)["id"],
				request.CloudAccountId, request.CloudProvider)
		},
	}
	handlers.Handle(w, r, cfg, http.StatusOK)
}
