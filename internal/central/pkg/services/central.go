...
// DinosaurService ...
//
//go:generate moq -out centralservice_moq.go . DinosaurService
type DinosaurService interface {
	...
	CountByStatus(status []constants.CentralStatus) ([]DinosaurStatusCount, error)
...
func (k *dinosaurService) RegisterDinosaurJob(ctx context.Context, dinosaurRequest *dbapi.CentralRequest) *errors.ServiceError {
	...
	dbConn := k.connectionFactory.New()
	dinosaurRequest.Status = constants.CentralRequestStatusAccepted.String()
	...
	metrics.UpdateCentralRequestsStatusSinceCreatedMetric(constants.CentralRequestStatusAccepted, dinosaurRequest.ID, dinosaurRequest.ClusterID, time.Since(dinosaurRequest.CreatedAt))
	return nil
}
...
// ListByStatus ...
func (k *dinosaurService) ListByStatus(status ...constants.CentralStatus) ([]*dbapi.CentralRequest, *errors.ServiceError) {
	if len(status) == 0 {
...
// UpdateStatus ...
func (k *dinosaurService) UpdateStatus(id string, status constants.CentralStatus) (bool, *errors.ServiceError) {
	dbConn := k.connectionFactory.New()
	...
	if dinosaur.Status == constants.CentralRequestStatusDeprovision.String() && status != constants.CentralRequestStatusDeleting {
...
// CountByStatus ...
func (k *dinosaurService) CountByStatus(status []constants.CentralStatus) ([]DinosaurStatusCount, error) {
	dbConn := k.connectionFactory.New()
