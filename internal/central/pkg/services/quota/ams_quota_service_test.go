...
func Test_AMSCheckQuota(t *testing.T) {
	...
	type args struct {
		...
		reserve             bool
		owner               string
		centralInstanceType types.CentralInstanceType
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			central := &dbapi.CentralRequest{
				Meta: api.Meta{
					ID: tt.args.centralID,
				},
				Owner:        tt.args.owner,
				InstanceType: string(tt.args.centralInstanceType),
			}
			standardAllowance, err := quotaService.HasQuotaAllowance(central, types.STANDARD)
			gomega.Expect(err).ToNot(gomega.HaveOccurred())
			evalAllowance, err := quotaService.HasQuotaAllowance(central, types.EVAL)
			gomega.Expect(err).ToNot(gomega.HaveOccurred())
			...
			_, err = quotaService.ReserveQuota(emptyCtx, central, "", "")
...
func Test_AMSReserveQuota(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			subID, err := quotaService.ReserveQuota(emptyCtx, central, "", "")
...
func Test_amsQuotaService_HasQuotaAllowance(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "returns false if no quota cost exists for the central's organization",
			ocmClient: &ocmClientMock.ClientMock{
				GetOrganisationFromExternalIDFunc: func(externalId string) (*v1.Organization, error) {
					org, _ := v1.NewOrganization().ID(fmt.Sprintf("fake-org-id-%s", externalId)).Build()
					return org, nil
			...
			args: args{
				centralRequest:      &dbapi.CentralRequest{OrganisationID: "central-org-1"},
				centralInstanceType: types.STANDARD,
			},
			want:    false,
			wantErr: false,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			res, err := quotaService.HasQuotaAllowance(tt.args.centralRequest, tt.args.centralInstanceType)
			gomega.Expect(err != nil).To(gomega.Equal(tt.wantErr))
...
func Test_amsQuotaService_HasQuotaAllowance_Extra(t *testing.T) {
	...
	for _, testcase := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := quotaService.HasQuotaAllowance(tt.central, types.CentralInstanceType(tt.central.InstanceType))
