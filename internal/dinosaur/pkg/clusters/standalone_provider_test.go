...
func TestStandaloneProvider_GetCloudProviders(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			RegisterTestingT(t)
			test.setupFn()
			provider := newStandaloneProvider(test.fields.connectionFactory, config.GetDataplaneClusterConfig())
...
func TestStandaloneProvider_buildOpenIDPClientSecret(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			RegisterTestingT(t)
			provider := newStandaloneProvider(db.NewMockConnectionFactory(nil), config.GetDataplaneClusterConfig())
