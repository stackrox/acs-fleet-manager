...
cluster, you will need to register it in the database so that it can be used by the service for incoming Central requests.  The cluster must have been created with multizone availability.
...
clusters:
  - name: cluster-name
    ...
    central_instance_limit: 2 # change this to match any value of configuration
...
fleet-manager allows provisioning of centrals in an already preexisting standalone dataplane cluster. To do so, add the standalone cluster in the [dataplane-cluster-configuration.yaml](../config/dataplane-cluster-configuration.yaml) giving the:
 ...
 - `cluster_dns` This will be used to build central host url and to communicate with clusters e.g `apps.example.dns.com`. This option is required.
...
> NOTE: [OLM](https://github.com/operator-framework/operator-lifecycle-manager#installation) in the destination standalone cluster/s is a prerequisite to be able to install central operator and fleetshard operators
...
2. Ensure the cluster is ready to be used for incoming Central requests.
    - Take note of the status of the cluster, `cluster_provisioned`, when you registered it to the database in step 2. This means that the cluster has been successfully provisioned but still have remaining resources to set up (i.e. Central operator installation).
    - Run the service using `make run` and let it reconcile resources required in order to make the cluster ready to be used by Central requests.
    - Once done, the cluster status in your database should have changed to `ready`. This means that the service can now assign this cluster to any incoming Central requests so that the service can process them.
