...
export FLEET_MANAGER_COMMAND="/usr/local/bin/fleet-manager serve --api-server-bindaddress=0.0.0.0:8000 --health-check-server-bindaddress=0.0.0.0:8083 --kubeconfig=/secrets/kubeconfig --enable-dinosaur-external-certificate=$ENABLE_DINOSAUR_EXTERNAL_CERTIFICATE --dinosaur-domain-name '$DINOSAUR_DOMAIN_NAME'"
